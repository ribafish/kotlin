...
TEST(RunLoopTimerTest, MainThread) {
    ...
    objc_support::RunLoopTimer timer(callback.AsStdFunction(), interval, initialTimeout);
...
TEST(RunLoopTimerTest, RepeatedTimer) {
    testing::StrictMock<testing::MockFunction<void()>> callback;
    objc_support::RunLoopTimer timer(callback.AsStdFunction(), std::chrono::microseconds(10), std::chrono::seconds(0));
...
TEST(RunLoopTimerTest, EffectivelyOneShotTimer) {
    testing::StrictMock<testing::MockFunction<void()>> callback;
    objc_support::RunLoopTimer timer(callback.AsStdFunction(), std::chrono::hours(10), std::chrono::microseconds(10));
    std::atomic<int> counter = 0;
...
TEST(RunLoopTimerTest, EffectivelyNeverTimer) {
    testing::StrictMock<testing::MockFunction<void()>> callback;
    objc_support::RunLoopTimer timer(callback.AsStdFunction(), std::chrono::hours(10), std::chrono::hours(10));
