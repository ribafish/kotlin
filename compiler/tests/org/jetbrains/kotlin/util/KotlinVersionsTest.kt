...
class KotlinVersionsTest : KtUsefulTestCase() {
    fun testVersionsAreConsistent() {
        ...
        val defaultVersion = LanguageVersion.DEFAULT.versionString.toVersion("LanguageVersion.DEFAULT")
        ...
        if (versions.any { it.isConsistentWith(defaultVersion) }) {
            Assert.fail(
                "Some versions are consistent with LATEST_STABLE ${defaultVersion.versionString}. Please edit KotlinVersionsTest accordingly:\n\n" +
                        versions.joinToString(separator = "\n") { with(it) { "$versionString ($source)" } }
            )
        }
    }
