...
class FirCallCompletionResultsWriterTransformer(
        ...
        class SamConversionInsertionTransformer : FirTransformer<Nothing?>() {
            override fun <E : FirElement> transformElement(element: E, data: Nothing?): E {
                ...
                if (transformed is FirExpression) {
                    ...
                    val functionType = expectedArgumentsTypeMapping?.samFunctionTypes?.get(key)
                    if (expectedArgumentType != null && functionType != null) {
                        @Suppress("UNCHECKED_CAST")
                        return transformed.wrapInSamExpression(expectedArgumentType, functionType) as E
                    }
                }
    ...
    private fun FirExpression.wrapInSamExpression(expectedArgumentType: ConeKotlinType, functionType: ConeKotlinType): FirExpression {
    ...
    private fun FirTypeRef.substitute(candidate: Candidate): ConeKotlinType =
        coneType.substitute(candidate)

    private fun ConeKotlinType.substitute(candidate: Candidate) =
    ...
    private fun Candidate.createArgumentsMapping(): ExpectedArgumentType.ArgumentsMap? {
        ...
        val arguments = argumentMapping?.flatMap { (argument, valueParameter) ->
            ...
            argument.unwrapAndFlattenArgument(flattenArrays = false).map {
                ...
                val functionType = functionTypesOfSamConversions?.get(it)
                if (functionType != null) {
                    if (samConversions == null) samConversions = mutableMapOf()
                    samConversions!![element] = functionType.substitute(this)
                }
