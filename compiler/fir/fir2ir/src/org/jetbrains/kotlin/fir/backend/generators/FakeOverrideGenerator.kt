...
class FakeOverrideGenerator(
    ...
    private inline fun <reified S : FirCallableSymbol<*>> computeBaseSymbolsWithContainingClass(
        ...
        return overriddenFirSymbols.mapNotNull { symbol ->
            with(typeContext) {
                val symbolDispatchReceiver = symbol.containingClassLookupTag() ?: return@mapNotNull null
                val superDispatchReceiver = superTypes.firstOrNull { superType ->
                    superType.anySuperTypeConstructor {
                        it.typeConstructor() == symbolDispatchReceiver
                    }
                } ?: return@mapNotNull null
                symbol to superDispatchReceiver.lookupTag
            }
        }
    }
