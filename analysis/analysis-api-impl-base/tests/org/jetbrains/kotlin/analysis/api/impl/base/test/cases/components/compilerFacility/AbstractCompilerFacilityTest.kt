...
abstract class AbstractCompilerFacilityTest : AbstractAnalysisApiBasedTest() {
    ...
    private fun findTargetFile(moduleStructure: TestModuleStructure, testServices: TestServices): Pair<TestModule, KtFile> {
        val isSingleModule = moduleStructure.modules.size == 1

        for (testModule in moduleStructure.modules) {
            val modulePsiFiles = testServices.ktModuleProvider.getModuleFiles(testModule)
            if (isSingleModule) {
                val ktFiles = modulePsiFiles.filterIsInstance<KtFile>()
                if (ktFiles.size == 1) {
                    // In simpler whole-file compilation tests, do not require the '<caret>'
                    return testModule to ktFiles.first()
                }
            }

            for (psiFile in modulePsiFiles) {
