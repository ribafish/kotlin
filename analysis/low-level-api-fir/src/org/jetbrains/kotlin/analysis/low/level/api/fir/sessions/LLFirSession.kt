...
abstract class LLFirSession(
    ...
    private class LLFirSessionCleaner(private val disposable: Disposable?) : SoftValueCleaner<LLFirSession> {
        override fun cleanUp(value: LLFirSession?) {
            ...
            // we cannot store the session in the session cleaner (otherwise the session will never be garbage-collected). The session
            // cleaner must rely on the `session` parameter provided by the soft value cache.
